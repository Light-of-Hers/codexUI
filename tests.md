...
- **步骤：**
  ...
  2. 点击触发按钮展开下拉面板。面板顶部显示当前 `cwd` 完整路径，下方为该目录的文件树。
  3. 确认根目录条目按“目录在前、文件在后、字母序”排列；点击一个目录行，确认其展开并懒加载子条目（首次展开时通过 `/codex-local-entries` 拉取，已加载的目录再次展开即时显示）。
  4. 点击一个文件行，确认在新标签页打开 `/codex-local-browse` 查看文件内容。
  5. 点击一个目录名，确认在新标签页打开该目录的 `/codex-local-browse` 列表；点击目录前的箭头符号，确认仅折叠/展开子树、不打开新标签页。
  6. 再次点击已展开目录的箭头符号，确认折叠其子树。
  7. 切换线程，确认浏览器自动重置为新区段的 `cwd` 并清空已缓存目录。
  8. 切换浅色主题重复步骤 1–6；再切换深色主题重复步骤 1–6。
- **预期结果：** 触发按钮在 `cwd` 为空时禁用；面板顶部显示 `cwd` 路径；目录懒加载、可展开/折叠（chevron 旋转）；文件点击在新标签页打开 local-browse；目录名点击在新标签页打开该目录，目录前箭头符号负责折叠/展开；目录切换时缓存清空、重新加载。浅色主题下面板为白底浅边框，深色主题下面板为 zinc-900 底、zinc-700 边框，目录/文件图标与文字清晰可读。
