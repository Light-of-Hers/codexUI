...
4. In the thread, inspect assistant and user messages, code fences, plan cards, Markdown tables, command groups, tool-call cards, and file-change summaries. Confirm Markdown tables use a square frame, muted-gray header, compact middle-aligned cells, and stronger horizontal than vertical dividers; expand and collapse commands/tool details and verify every control remains usable, the latest command in the group summary remains monospace, and the expanded command/output frames are square.
...
#### Performance Audit
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_WAIT_MS=7000 pnpm run profile:browser` recorded an 8.07 s home-route baseline with 292.3 KB API payload. The profiler reported existing `threadRead=10`, `skillsList=3`, `rateLimitsRead=3`, and `providerModels=1093.3ms` warnings; the slowest row is `thread/read` at 3563.8 ms (about 1.0 KB response), with no added theme request or payload.
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_ROUTE="#/thread/019da7c0-4e12-7a91-837c-f7c11cc8ab6c" PROFILE_WAIT_MS=7000 pnpm run profile:thread` recorded a 7.42 s direct-thread baseline with 182.6 KB API payload. It reported only the existing `threadRead=11` warning; the slowest row was `thread/read` at 2732.7 ms. The styling change adds no request or payload.
