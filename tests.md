...
#### Performance Audit
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_WAIT_MS=7000 pnpm run profile:browser` recorded a 7.93 s home-route baseline with 228.0 KB API payload. The profiler reported existing `threadRead=10`, `skillsList=2`, and `rateLimitsRead=2` warnings; the slowest row is `thread/read` at 3200.3 ms (about 1.0 KB response), with no added table-style request or payload.
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_ROUTE="#/thread/019da7c0-4e12-7a91-837c-f7c11cc8ab6c" PROFILE_WAIT_MS=7000 pnpm run profile:thread` recorded a 7.44 s direct-thread baseline with 181.8 KB API payload. It reported the existing `threadRead=11` warning; the slowest row was `thread/read` at 2586.7 ms. The styling change adds no request or payload.
