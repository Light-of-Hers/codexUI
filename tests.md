...
3. Inspect the sidebar, settings panel, thread header, dropdowns, and new-thread cards. Confirm that box-like controls use square edges, the header keeps its cyan accent rule above a neutral gray divider, and selected/primary actions use cyan without obscuring text.
...
#### Performance Audit
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_WAIT_MS=7000 pnpm run profile:browser` recorded an 8.39 s home-route baseline with 180.8 KB API payload. The profiler reports only the existing `threadRead=9` warning; the slowest row is `thread/read` at 3156.6 ms (about 1.0 KB response), with no added theme request or payload.
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_ROUTE="#/thread/019da7c0-4e12-7a91-837c-f7c11cc8ab6c" PROFILE_WAIT_MS=7000 pnpm run profile:thread` recorded a 7.43 s direct-thread baseline with 181.1 KB API payload. It reports only the existing `threadRead=10` warning; the slowest row is the pre-existing `thread/read` at 2325.8 ms. The styling change adds no request or payload.
