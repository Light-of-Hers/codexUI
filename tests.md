...
#### Steps
1. In Light appearance, open the home screen and a populated thread. Confirm the lighter cool-gray canvas uses the subtle grid texture, square panel edges, bright-cyan primary rules, and fixed blue-gray structural accents while the existing sidebar, composer, and conversation layout stay in place.
...
#### Performance Audit
- `PROFILE_BASE_URL=http://127.0.0.1:4173 PROFILE_WAIT_MS=7000 pnpm run profile:browser` recorded an 8.33 s home-route baseline with 226.8 KB API payload. The profiler reports `threadRead=9`, `skillsList=2`, and `rateLimitsRead=2` warnings; these are existing startup requests outside this CSS-only theme path. The slowest row is `thread/read` at 3124.0 ms (about 1.0 KB response), with no added theme request or payload.
