...
- **步骤：**
  ...
  2. 点击触发按钮展开下拉面板。面板分为两组：上方 `Links` 区列出所有网页 URL，下方 `Files` 区以 file explorer 树形展示文件路径。
  3. 在 `Files` 树中确认公共前缀已合并为目录节点：例如 `/root/proj/src/app.ts` 与 `/root/proj/src/utils/helpers.ts` 共享 `/root/proj/src/` 前缀，`src` 为目录节点、`app.ts` 与 `utils` 为其子项。默认仅展开顶级目录，深层目录折叠；点击目录行可展开/折叠其子项。
  4. 在搜索框输入关键字（如 `merlin`、`https`），确认 web 与文件结果即时过滤；搜索时文件以扁平列表展示匹配项。
  5. 点击一条 web 链接，确认在新标签页打开；点击一条文件路径，确认跳转 `/codex-local-browse` 路由；点击文件行的复制按钮，确认路径已复制到剪贴板。
  6. 点击面板外部或按 `Esc` 关闭面板。
  7. 切换浅色主题重复步骤 1–6；再切换深色主题重复步骤 1–6。
- **预期结果：** 触发计数基于完整历史而非当前窗口（同一线程切换回来时计数稳定，不会因滚动加载更多消息而变化）；网页 URL 全部集中在 `Links` 区、位于文件之前；文件路径按公共前缀合并为可展开的目录树，目录在前、文件在后、字母序排列；相同 URL/路径只出现一次；`file://` URI 与行号后缀被规范化为纯路径；纯数字、版本号、端口号、代码标识符等非路径 token 不被收录；搜索、展开/折叠、复制均可用。浅色主题下面板为白底浅边框，深色主题下面板为 zinc-900 底、zinc-700 边框，目录/文件图标与文字清晰可读。
...
- **步骤：**
  ...
  3. 面板顶部 `cwd` 行右侧有一个眼睛图标按钮（显式隐藏文件开关），默认关闭（隐藏以 `.` 开头的文件/目录）；点击切换为显示，已展开目录会按新设置重新拉取；再次点击关闭。
